...
const NormalPeriodOfStudyIntentHandler = {
    canHandle(handlerInput){
        const res = handlerInput.requestEnvelope.request;

        return res.type === 'IntentRequest' &&
                res.intent.name === 'NormalPeriodOfStudyIntent';
    },
    handle(handlerInput){
        
        const data = require('./../APL/normalPeriodOfStudyData.json');
        const template = require('./../APL/longTextTemplate.json');
        
        const speechOutput = speechOutJson[0].normalPeriodOfStudy + randomizeFunction(questionPossibility);
...
module.exports = NormalPeriodOfStudyIntentHandler;