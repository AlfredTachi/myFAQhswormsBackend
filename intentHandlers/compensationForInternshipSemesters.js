...
const CompensationForInternshipSemestersIntentHandler = {
    canHandle(handlerInput){
        const res = handlerInput.requestEnvelope.request;

        return res.type === 'IntentRequest' &&
                res.intent.name === 'CompensationForInternshipSemestersIntent';
    },
    handle(handlerInput){
        ...
        const speechOutput = speechOutJson[0].compensationForInternshipSemesters + randomizeFunction(questionPossibility);
...
module.exports = CompensationForInternshipSemestersIntentHandler;